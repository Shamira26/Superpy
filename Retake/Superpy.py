...
def main():
    today = HandleDate()
    csv_handler = HandleCSV("Transaction_log.csv")
    ...
    subparsers = parser.add_subparsers(title="subcommands",dest="subcommand", description="valid subcommands", help="The three main subcommands")
    ...
    report_subparsers = report_parser.add_subparsers(title="subcommands",dest="report_subcommand",description="valid subcommands", help="Additional help for report purposes.")

    # Subparser for the 'inventory' subcommand
    inventory_parser = report_subparsers.add_parser("inventory", help="Inventory report")
    ...
    inventory_parser.add_argument("--start-date", type=str, help="Specify a start date for inventory purposes. Format: YYYY-MM-DD")
    inventory_parser.add_argument("--end-date", type=str, help="Specify an end date for inventory purposes. Format: YYYY-MM-DD")

    # Subparser for the 'revenue' subcommand
    revenue_parser = report_subparsers.add_parser("revenue", help="Revenue report")
    revenue_parser.add_argument("--date", type=str, help="Specify a date for revenue purposes. Format: YYYY-MM-DD")

    # Subparser for the 'profit' subcommand
    profit_parser = report_subparsers.add_parser("profit", help="Profit report")
    profit_parser.add_argument("--date", type=str, help="Specify a date for profit purposes. Format: YYYY-MM-DD")
    ...
    # Set a specific date as the current date
    parser.add_argument("--advanced-time", type=int, help="Advance the current date by a specified number of days.Format: YYYY-MM-DD")
    parser.add_argument("--set-date", type=str, help="Set the current date to the specified date.Format: YYYY-MM-DD")
    
    args=parser.parse_args()

    if args.advanced_time:
        today.advanced(args.advanced_time)
        buy_instance.update_status()    
