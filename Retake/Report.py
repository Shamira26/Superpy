...
class report:
    ...
    def inventory(self, start_date=None, end_date=None, inventory_type=None):
        ...
        if not transactions:
            ...
            for row in transactions:
                table.add_row(row['Product_name'], str(row['Quantity']), str(row['Original_quantity']),str(row['Price']), row['Date'], row['Expiration_date'], row['Status'])
    ...
    def get_cost(self, date = None):
        ...
        total_cost = sum((float(row["Original_quantity"])*float(row["Price"])) for row in actions_on_date)
        return total_cost
    ...
    def get_profit(self, date=None):
        ...
        total_cost = sum(float(row["Original_quantity"]) * float(row["Price"]) for row in actions_on_date if row["Type"] == "Buy")
        # Calculate profit
        profit = revenue - total_cost
